...
public class MBF {
	...
	private int[] mbfPlacement;
	private double mbf_energy;
	public double ff_bw;
	...
	public int[] MBF()
	{
		this.mbfPlacement = new int[numVM];
		...
		for(int i = 0; i < numVM; i++)
		{
			...
			ArrayList serverList = this.getServerList(i,useCPU,useMEM,mbfPlacement);
			
			for(int j = 0 ; j < serverList.size(); j++)
			{
				int lastPm = mbfPlacement[vmr.id];
				int pm = (int) serverList.get(j);
				mbfPlacement[vmr.id] = pm;
				
				double fitness =  sumTraffic(mbfPlacement) + this.totalPmEnergy(mbfPlacement) + this.totalSwitchEnergy(this.mbfPlacement);
				...
				{
					mbfPlacement[vmr.id] = lastPm;
				}
				
			}
			
			this.updateLinkLoadByFFD(i, mbfPlacement[vmr.id], this.vmList, mbfPlacement);
			this.updatePmResource(vmr.id, mbfPlacement[vmr.id], useCPU, useMEM);
		}
		
		double pmEnergy = this.totalPmEnergy(mbfPlacement);
		double switchEnergy = this.totalSwitchEnergy(mbfPlacement);
		
		this.mbf_energy =  pmEnergy + switchEnergy ;
	
		return this.mbfPlacement;	
