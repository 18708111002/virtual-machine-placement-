...
public class BPBFD {
	...
	public int [] BPBFD()
	{
		...
		for(int i = 0; i < numVM; i++)
		{	
			ArrayList serverList = this.getServerList(i,useCPU,useMEM,ffPlacement);
			double minRemain = Double.MAX_VALUE;
			
			for(int j = 0 ; j < serverList.size(); j++)
			{
				int lastPm = ffPlacement[i];
				int pm = (int) serverList.get(j);
				ffPlacement[i] = pm;
				
				double remainCPU = this.pmCPU[pm] - useCPU[pm];
				
				if(remainCPU < minRemain)
				{
					minRemain = remainCPU;
//					this.updateLinkLoadByFF(i, pm, ffPlacement);
					this.updatePmResource(i, pm, useCPU, useMEM);
				}
				else
				{
					ffPlacement[i] = lastPm;
				}
	...
	private ArrayList getServerList(int vm, int[] useCPU, int[] useMEM, int[] placement) // return servers can hold the VM.
	{
		...
		for(int j = 0; j < numPM; j++)
		{
			if(	this.isResouceSatisfy(vm,j,useCPU,useMEM) &&
				this.isNetworkSatisfy(vm,j,placement)	)
			{
				serverList.add(j);
			}
