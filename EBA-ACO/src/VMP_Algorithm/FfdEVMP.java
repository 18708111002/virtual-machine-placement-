...
public class FfdEVMP {
	...
	public int [] FFD()
	{
		...
		for(int i = 0; i < numVM; i++)
		{
			...
			int  server = this.getServer(i,useCPU,useMEM,ffdPlacement); // servers can hold the current VM i.
			
			if(server > -1)
			{
				ffdPlacement[vmr.id] = server;
				this.updateLinkLoadByFFD(i, server, this.vmList, ffdPlacement);
...
//				System.out.println("no hold server");
		...
		double pmEnergy = this.totalPmEnergy(ffdPlacement);
		double switchEnergy = this.totalSwitchEnergy(ffdPlacement);
		
		this.ffd_energy =  pmEnergy + switchEnergy ;
		
		java.text.DecimalFormat df = new java.text.DecimalFormat("#.00");
		this.ffd_bw = Double.valueOf(df.format(this.useMeanSwitch(ffdPlacement)));
...
//				total += this.SWITCH_IDLE_ENERGY;
	...
	public double getFfdTotalEnergy()
	{
		return this.ffd_energy;
