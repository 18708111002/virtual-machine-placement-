...
public class MBFD {
	...
	private int[] bfdPlacement;
	private double bfd_energy;
	public double ffd_bw;
	...
	private ArrayList<vmRequire> vmList ;
	
	public MBFD(int vm,int pm)
	{
		this.numVM = vm;
		this.numPM = pm;
	...
	public int [] MBFD()
	{
		this.bfdPlacement = new int[numVM];
		...
		for(int i = 0; i < numVM; i++)
		{
			vmr = (vmRequire) vmList.get(i);
			double minPower = Double.MAX_VALUE;
			
			ArrayList serverList = this.getServerList(i,useCPU,useMEM,bfdPlacement);
			
			for(int j = 0 ; j < serverList.size(); j++)
			{
				int lastPm = bfdPlacement[vmr.id];
				int pm = (int) serverList.get(j);
				bfdPlacement[vmr.id] = pm;
				
				double power = this.totalPmEnergy(bfdPlacement) + this.totalSwitchEnergy(this.bfdPlacement);
				...
				{
					bfdPlacement[vmr.id] = lastPm;
				}
				
			}
			
			this.updateLinkLoadByFFD(i, bfdPlacement[vmr.id], this.vmList, bfdPlacement);
			this.updatePmResource(vmr.id, bfdPlacement[vmr.id], useCPU, useMEM);
		}
		
		double pmEnergy = this.totalPmEnergy(bfdPlacement);
		double switchEnergy = this.totalSwitchEnergy(bfdPlacement);
		
		this.bfd_energy =  pmEnergy + switchEnergy ;
	...
	{
		int vm = this.vmList.get(index).id;
	...
	public int [] FFD()
	{
		...
		for(int i = 0; i < numVM; i++)
		{
			...
			int  server = this.getServer(i,useCPU,useMEM,bfdPlacement); // servers can hold the current VM i.
			
			if(server > -1)
			{
				bfdPlacement[vmr.id] = server;
				this.updateLinkLoadByFFD(i, server, this.vmList, bfdPlacement);
...
//				System.out.println("no hold server");
		...
		this.ffd_bw = Double.valueOf(df.format(this.useMeanSwitch(bfdPlacement)));
	...
	{
		this.vmList = new ArrayList<vmRequire>();
		...
		vmList.sort(new SortByCPU());
		
	}
	
	private int getServer(int index, int[] useCPU, int[] useMEM, int[] placement) // return servers can hold the VM.
	{
		int vm = this.vmList.get(index).id;	
		...
		for(int i = 0; i < numPM; i++)
		{
			if(	this.isResouceSatisfy(vm,i,useCPU,useMEM) &&
				this.isNetworkSatisfy(index,i,placement)	)
			{
	...
	{
		int vm1 = this.vmList.get(index).id;
		
		for(int i = 0 ; i < index; i++)
		{
			int vm2 = this.vmList.get(i).id;
...
//			return true;
	...
	private boolean isPELinkOverLoad(int edge,int podNum,int pm,double traffic)
	{
		...
		int index = this.getPmIndex(podNum, edge, pm); //pm - (podNum * fatTreePod * fatTreePod / 4 + edge * fatTreePod / 2);

		if(pod.pLinkE[index][edge] + traffic > this.PM_EDGE_CAPACITY)
		{
//			System.out.println("pm" + index +" link edge overload:" + (pod.pLinkE[index][edge] + traffic) );
			return true;
		}
...
//				total += this.SWITCH_IDLE_ENERGY;
	...
	public double getBfdTotalEnergy()
	{
		return this.bfd_energy;
