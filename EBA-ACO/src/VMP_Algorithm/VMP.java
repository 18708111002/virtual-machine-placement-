...
//	private final static double CORE_SWITCH_FULL_ENERGY = 650 ;
	...
	public VMP(int numVm, int numPm,String fileName,String file) throws IOException
	{
		this.numPM = numPm;
		this.numVM = numVm;
...
//			System.out.println("infeasible");
	...
	private int calculateAntsFitness(int ants, int[][] antsSolution,double [] antsFitness) {
		...
		for(int i = 0; i < ants; i++)
		{
			antsFitness[i] = (this.SERVER_FULL_ENERGY ) / (this.totalPmEnergy(antsSolution[i]) + sumTraffic(antsSolution[i]));
...
//		System.out.println("aco:" + sumTraffic);
	...
	public static void generateSence(int vm,int pm) throws IOException
	{
		...
		for(;; ) {
			...
			VMP aco_evmp = new VMP(VM,PM,fileName,file);
