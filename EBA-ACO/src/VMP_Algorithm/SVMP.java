...
public class SVMP {
	...
	private final static double RHO_GLOBAL = 0.5;
	...
	private ArrayList<vmRequire> pmList;

	

	public SVMP(int numVm, int numPm) 
	{
...
//				System.out.println("no hold server");
	...
	private void generatePmList()
	{
		this.pmList = new ArrayList<vmRequire>();
		
		boolean [] isAdd = new boolean[this.numPM];
		...
		while(this.pmList.size() < this.numPM)
		{
			int pm = random.nextInt(this.numPM);
			
			while(isAdd[pm])
				pm = random.nextInt(this.numPM);
			
			isAdd[pm] = true;
			
			vmRequire pmr = new vmRequire(pmCPU[pm],pmMEM[pm],pm);
			this.pmList.add(pmr);
		}
	}
	...
	public int [] SVMP()
	{
		...
		for(int i = 0; i < numVM; i++)
		{
			for(int j = 0; j < this.pmList.size(); j++)
			{
				if(	this.isResouceSatisfy(i,this.pmList.get(j).id,useCPU,useMEM) &&
						this.isNetworkSatisfy(i,this.pmList.get(j).id,placement))
				{
					placement[i] = this.pmList.get(j).id;
					
					this.updatePmResource(i,placement[i],useCPU,useMEM);
					this.updateLinkLoad(i,this.pmList.get(j).id,placement);
				}
			}
		}
	
//		this.placeDetil("init ", placement);
		System.arraycopy(placement, 0, this.bestPlacement, 0, numVM);
		this.curBestEnergy = this.totalEnergy(this.bestPlacement);
		...
		while(T0 > 0)
		{
			for(int i = 0 ; i < Tmax; i++)
			{
				...
				if(r==0)
				{
					this.inversion(placement);
					
				}
				else if (r == 1)
				{
					this.tranlation(placement);
				}
				else if(r == 2)
				{
					this.switching(placement);
				}
			}
			...
			for(int i = 0; i < numVM; i++)
				this.updateLinkLoadByVM(i, placement[i], placement);
			
			if(this.isFeasible(placement))
			{
				
				double energy = this.totalEnergy(placement);
				
				if(energy < this.curBestEnergy)
				{
					System.arraycopy(placement, 0, this.bestPlacement, 0, numVM);
					this.curBestEnergy = energy;
				}
				else if(energy > this.curBestEnergy)
				{
					double theta = Math.random();
					double tk = theta * T0;
					
					if(theta < Math.exp((energy - this.curBestEnergy)* 1.0 / (tk * 1.0)))
					{
						System.arraycopy(placement, 0, this.bestPlacement, 0, numVM);
						this.curBestEnergy = energy;
			...
			for(int j = 0; j < numVM; j++)
				placement[j] =  new Random().nextInt(numPM);
...
//		System.out.println("GA"  +  switchEnergy);
	...
	private void tranlation(int[] placement)
	{
		Random random = new Random();
		
		int [] s = new int [numVM];
		int j = 0;
		int s1 = random.nextInt(numVM);
		int l1 = random.nextInt(numVM - s1);
		
		for(int i = 0; i < numVM ; i ++)
		{
			if(i < s1 || (s1 < i && i <= s1 + l1))
			{
				s[j++] = placement[i];
...
//			System.out.println(j);
		...
		System.arraycopy(s, 0, placement, 0, numVM);
		
	}// end of tranlation
	
	private void switching(int[] placement)
	{
		int i = new Random().nextInt(numVM);	
		int j = new Random().nextInt(numVM);	
		int pm = placement[i];
	...
	private void inversion(int [] placement)
	{
		Random random = new Random();
		...
		int s1 = random.nextInt(numVM);
		int l1 = random.nextInt(numVM - s1);
		int I1 = s1 + random.nextInt(s1 + l1 + 1);
...
//		total += this.calcaulateEASwitch(pod);
	...
	private void updateGlobalPheromone(int[] placement, double[][] pheromone, double curBestEnergy)
	{
		...
		boolean []activePm = new boolean[numPM];
		 ...
		 {
			 int pm = placement[i];
			 pheromone[i][pm] = (1 - this.RHO_GLOBAL)* pheromone[i][pm] + this.RHO_GLOBAL * numVM/(activeCount*1.0);	 
		 }	
...
//			System.out.println("aggre link core overload:" + (pod.eLinkA[aggre][core] + traffic) );
	...
	private void updateHeuristic(ArrayList serverList,int index, double[][] heuristic, int[] useCPU, int[] useMEM, boolean[] usePM, int[] placement) 
	{
		...
		{
			int pm = (int)server;
			double heuristicOne =  0.5 * this.heuristicOne(pm, vm, useCPU, useMEM, usePM) ;// resource balance.
			double heuristicTwo =  0.5 * this.heuristicTwo(pm, vm, useCPU, useMEM, usePM, placement);// server energy.
	...
	private double heuristicOne(int pm,int vm, int[] useCPU, int[] useMEM,boolean[] usePM)// resource balance.
	{
		double sum = 0;
		double rCPU,rMEM;
		double H = (this.pmCPU[pm] + this.pmMEM[pm]) /2.0;
		double part = StrictMath.pow((Math.pow(useCPU[pm] - H , 2) +  Math.pow(useMEM[pm] - H , 2)),0.5);
		
		return 	Math.abs(Math.log10(part));
	}// end of heuristicOne
	private double heuristicTwo(int pm,int vm, int[] useCPU, int[] useMEM,boolean[] usePM, int[] placement)// server energy.
	{
		return (useCPU[pm] + this.vmCPU[vm])/(this.pmCPU[pm] * 1.0) + (useMEM[pm] + this.vmMEM[vm]) / (this.pmMEM[pm] * 1.0);
